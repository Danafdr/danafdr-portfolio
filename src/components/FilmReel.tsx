...
export default function FilmReel() {
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (index !== currentVIndex) {
              ...
              const hContainer = hContainersRef.current[index - 1]; // Offset by 1 for Intro slide
              if (hContainer) hContainer.scrollLeft = 0;
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            // Animate Panel 2 content when it comes into view (ONLY ONCE per category)
            if (hIndex === 1) {
  ...
  return (
    <>
      ...
      <div className="fixed left-[20px] top-1/2 -translate-y-1/2 flex flex-col gap-[8px] z-40">
      ...
      <div className="fixed bottom-[36px] left-1/2 -translate-x-1/2 flex gap-[10px] z-40">
        {[0, 1].map((_, idx) => (
          <div
            key={idx}
            className={`w-[6px] h-[6px] rounded-full transition-all duration-200 ease-out
        ...
        {categories.map((category, catIdx) => (
          ...
          >
            ...
            >
                  ...
                  <div>
                    ...
                    <div className="anim-el">
                      <button
                        ...
                        className="text-[11px] font-mono tracking-[0.1em] uppercase text-ink2 flex items-center gap-2 cursor-pointer group w-fit bg-transparent border-none p-0"
                        role="button"
                      >
                        <span className="transition-colors group-hover:text-accent">
                          Browse projects
                        </span>
                        <span className="transition-transform group-hover:translate-x-1 group-hover:text-accent">
                          →
                        </span>
                      </button>
                    </div>
              ...
              >
                {/* Panel Header */}
                <div className="px-10 pt-8 pb-5 border-b border-[rgba(15,14,11,0.1)] flex items-end justify-between shrink-0">
                  <div>
                    <div className="text-[9px] text-accent tracking-[0.2em] uppercase mb-2">
                      {category.name}
                    </div>
                ...
                {/* Project List */}
                <div className="flex-1 overflow-y-auto no-scrollbar px-10 py-6" style={{ scrollbarWidth: "none" }}>
                  <div className="max-w-[1200px] mx-auto flex flex-col gap-0">
                    {category.projects.map((project) => (
                      <button
                        key={project.id}
                        onClick={() => setPreview(project)}
                        className="project-row w-full grid grid-cols-[60px_1fr_auto] gap-6 items-center py-6 border-b border-[rgba(15,14,11,0.08)] transition-all duration-200 group hover:bg-[rgba(15,14,11,0.02)] cursor-pointer text-left bg-transparent"
                        role="button"
                      >
                        {/* Thumbnail */}
                        <div
                          className="w-[60px] h-[40px] border border-[rgba(15,14,11,0.1)] overflow-hidden flex items-center justify-center transition-transform duration-300 group-hover:scale-105"
                          style={{
                            background: `linear-gradient(135deg, ${project.gradientStart}, ${project.gradientEnd})`,
                          }}
                        >
                          <div className="font-playfair font-black text-[7px] text-[#f0ebe2]/50 tracking-[-0.02em]">
                            {project.title}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-[14px] text-ink3 transition-all duration-200 group-hover:translate-x-1 group-hover:text-accent">
                            →
                          </span>
                        </div>
                      </button>
      ...
      {preview && (
        ...
        >
          ...
          >
            <button
              onClick={() => setPreview(null)}
              className="absolute top-5 right-5 z-20 w-8 h-8 flex items-center justify-center text-ink/60 hover:text-accent transition-colors text-[16px] font-mono cursor-pointer border border-[rgba(15,14,11,0.15)] bg-paper"
              role="button"
            >
              ×
            </button>
            ...
            <div className="p-8 md:p-10">
              <div className="flex items-start justify-between gap-8 mb-6">
                <div>
                  ...
                  <h3 className="font-playfair font-black text-[clamp(28px,4vw,44px)] leading-[0.9] tracking-[-0.02em] mb-4">
                    {preview.title}
                  </h3>
                </div>
                <div className="text-[11px] font-mono text-ink3 shrink-0 pt-6">
                  {preview.year}
                </div>
              </div>
              ...
              <div className="flex items-center gap-8 border-t border-[rgba(15,14,11,0.1)] pt-6">
                ...
                {preview.link ? (
                  <a
                    ...
                    className="ml-auto text-[11px] font-mono tracking-[0.1em] uppercase text-accent flex items-center gap-2 hover:gap-3 transition-all no-underline"
                  >
                    Visit live ↗
                  </a>
                ) : (
                  <div className="ml-auto text-[11px] font-mono tracking-[0.1em] uppercase text-ink3 flex items-center gap-2">
